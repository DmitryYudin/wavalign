...
static char testPref[512] = {
    '\0',
};
#define TRACE_ERR(cond, fmt, ...) \
    if (cond) { \
        fprintf(stderr, "%s: (%u): " fmt "\n", testPref, __LINE__, ##__VA_ARGS__); \
        return 1; \
    }
...
static const struct {
    ...
    {"sample_32b_24float_non_standard.wav", __sample_32b_24float_non_standard_wav,
     __sample_32b_24float_non_standard_wav_len},
...
static int test_readSmpl(const char *filename)
{
    ...
    TRACE_ERR(wr->samples_per_channel * wr->channels < COUNT_OF(pcm_data), "Wrong samples number: %u",
              wr->samples_per_channel)

    unsigned bytesTotal = wr->samples_per_channel * wr->block_align;
    unsigned char *data = new (std::nothrow) unsigned char[bytesTotal];
    TRACE_ERR(wr->samples_per_channel != WR_readRaw(wr, data, wr->samples_per_channel), "Error reading data: %s",
              filename)
...
static int test_readBuf(const char *filename, SmplFmt smpl_fmt, unsigned char *_data, unsigned *_samples_channel)
{
    ...
    TRACE_ERR((wr->samples_per_channel - 2) * wr->channels == COUNT_OF(pcm_data), "Wrong samples number: %u",
              wr->samples_per_channel)
