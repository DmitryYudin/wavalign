...
static int check_consistency()
{
    ...
    WR* wr = NULL;
    return &wavReader->format == &wr->format && &wavReader->channels == &wr->channels &&
           &wavReader->sample_rate == &wr->sample_rate && &wavReader->bits_per_sample == &wr->bits_per_sample &&
           &wavReader->block_align == &wr->block_align && &wavReader->samples_per_channel == &wr->samples_per_channel;
}
    ...
    while (len >= 8) {
        ...
        if (chunk == TAG('f', 'm', 't', ' ')) {
            ...
            if (wr->format == WAVE_FORMAT_EXTENSIBLE) {
                ...
                int cbSize = read_int16(wr);
                chunk_len -= 2;
                if (cbSize == 2) {
                    unsigned audition = read_int16(wr);
                    chunk_len -= 2;
                    if (audition == 1) {
            ...
            if (wr->format != WAVE_FORMAT_PCM && wr->format != WAVE_FORMAT_IEEE_FLOAT &&
                wr->format != WAVE_FORMAT_FLOAT_AUDITION) {
                break;
            }
